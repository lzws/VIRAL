...
from ..models.qwen_image_mot import MotQwenModel
...
class QwenImagePipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16):
        ...
        self.in_iteration_models = ("dit", "blockwise_controlnet", "mot")
        self.units = [
...
class QwenImageUnit_PromptEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            ...
            input_params=("edit_image","if_prompt",),
    ...
    def encode_prompt_edit_multi(self, pipe: QwenImagePipeline, prompt, edit_image,if_prompt):
        p = "According to the transformation from Picture 1 to Picture 2, perform the same transformation on Picture 3 to generate an output. "
        template =  "<|im_start|>system\nDescribe the key features of the input image (color, shape, size, texture, objects, background), then explain how the user's text instruction should alter or modify the image. Generate a new image that meets the user's requirements while maintaining consistency with the original input where appropriate.<|im_end|>\n<|im_start|>user\n{}<|im_end|>\n<|im_start|>assistant\n"
        drop_idx = 64
        ...
        # print(f'edit_image: {len(edit_image)}')
        model_inputs = pipe.processor(text=txt, images=edit_image, padding=True, return_tensors="pt").to(pipe.device)
        
        # ic_idx_end = torch.where(model_inputs.input_ids[0]==151653)[-1][-1]

        ic_idx_start = drop_idx
        ic_idx_end = -1

        hidden_states = pipe.text_encoder(input_ids=model_inputs.input_ids, attention_mask=model_inputs.attention_mask, pixel_values=model_inputs.pixel_values, image_grid_thw=model_inputs.image_grid_thw, output_hidden_states=True,)[-1]
        split_hidden_states_ = self.extract_masked_hidden(hidden_states, model_inputs.attention_mask)
        split_hidden_states = [e[drop_idx:] for e in split_hidden_states_]
        ic_hidden_states = [e[ic_idx_start:] for e in split_hidden_states_]
        return split_hidden_states, ic_hidden_states

    def process(self, pipe: QwenImagePipeline, prompt, edit_image=None, if_prompt=False) -> dict:
        if pipe.text_encoder is not None:
            ...
            if edit_image is None:
                ...
                split_hidden_states = self.encode_prompt_edit_multi(pipe, prompt, edit_image,if_prompt)
            res = {}
            if isinstance(split_hidden_states,tuple):
                split_hidden_states, ic_hidden_states = split_hidden_states
                ic_mask_list = [torch.ones(e.size(0), dtype=torch.long, device=e.device) for e in ic_hidden_states]
                max_ic_seq_len = max([e.size(0) for e in ic_hidden_states])
                ic_prompt_embeds = torch.stack([torch.cat([u, u.new_zeros(max_ic_seq_len - u.size(0), u.size(1))]) for u in ic_hidden_states])
                ic_prompt_emb_mask = torch.stack([torch.cat([u, u.new_zeros(max_ic_seq_len - u.size(0))]) for u in ic_mask_list])
                ic_prompt_embeds = ic_prompt_embeds.to(dtype=pipe.torch_dtype, device=pipe.device)
                res.update({"ic_prompt_emb": ic_prompt_embeds, "ic_prompt_emb_mask": ic_prompt_emb_mask})
            ...
            res.update({"prompt_emb": prompt_embeds, "prompt_emb_mask": encoder_attention_mask})
            return res
        else:
            return {}
...
class QwenImageUnit_EditImageEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("edit_image", "tiled", "tile_size", "tile_stride","width", "height", "edit_nums", "edit_image_auto_resize"),
            onload_model_names=("vae",)
        )
    ...
    def process(self, pipe: QwenImagePipeline, edit_image, tiled, tile_size, tile_stride, width, height, edit_nums, edit_image_auto_resize=False):
        if edit_image is None:
            return {}
        pipe.load_models_to_device(['vae'])
        if isinstance(edit_image, Image.Image):
            ...
            ewidth, eheight = 1024, 1024
            for image in edit_image[-edit_nums:]:
            ...
            ic_latents_list = []
            if pipe.mot is not None and len(edit_image)==3:
                for j,image in enumerate(edit_image[:2]):
                    if j == 1:
                        image = image.resize((ewidth, eheight))
                    if j == 0:
                        image = image.resize((width, height))
                    image = pipe.preprocess_image(image).to(device=pipe.device, dtype=pipe.torch_dtype)
                    latents = pipe.vae.encode(image, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
                    ic_latents_list.append(latents)
        return {"edit_latents": edit_latents, "edit_image": resized_edit_image, "ic_latents_list":ic_latents_list}
...
def model_fn_qwen_image(
    ...
    mot=None,
    ic_latents_list=None,
    ic_prompt_emb_mask=None,
    ic_prompt_emb=None,
    **kwargs
):
    img_shapes = [(latents.shape[0], latents.shape[2]//2, latents.shape[3]//2)]
    # print(f"latents 1: {latents.shape}")
    # print(f'img_shapes 1: {img_shapes}') # [(1, 64, 64)]
    ...
    if edit_latents is not None:
        ...
        # print(f"e 2:{edit_latents_list[0].shape}")
        # print(f"img_shapes 2: {img_shapes}") # [(1, 64, 64), (1, 64, 64)]
    ...
    if entity_prompt_emb is not None:
        ...
        if edit_rope_interpolation:
            ...
            # print(f"image_rotary_emb 1: {image_rotary_emb[0].shape}")
            # print(f"image_rotary_emb 1-1: {image_rotary_emb[1].shape}")
        attention_mask = None

    if mot is not None:
         # incontext temb
        ic_temb = mot.time_text_embed(torch.tensor([20 / 1000]).to(image.device), image.dtype)
        # incontext image
        ic_img_shapes = [(e.shape[0], e.shape[2]//2, e.shape[3]//2) for e in ic_latents_list]
        # print(f'ic_img_shapes 1: {ic_img_shapes}')

        ic_img = [rearrange(e, "B C (H P) (W Q) -> B (H W) (C P Q)", H=e.shape[2]//2, W=e.shape[3]//2, P=2, Q=2) for e in ic_latents_list]
        ic_img = torch.cat(ic_img, dim=1)
        # print(f"ic_img 1: {ic_img.shape}")
        ic_img = mot.img_in(ic_img)

        # print(f"ic_img 2: {ic_img.shape}")

        # incontext text
        ic_txt_seq_lens = ic_prompt_emb_mask.sum(dim=1).tolist()
        ic_txt = mot.txt_in(mot.txt_norm(ic_prompt_emb))
        # print(f"ic_txt 1: {ic_txt.shape}")
        # rope
        ic_img_rotary_emb = mot.pos_embed(ic_img_shapes, ic_txt_seq_lens, device=latents.device)
        # print(f"ic_img_rotary_emb 1: {ic_img_rotary_emb[0].shape}")
        # print(f"ic_img_rotary_emb 1-1: {ic_img_rotary_emb[1].shape}")

        # compute mot hiddenstate
        text_hints, image_hints = mot(image, ic_img, ic_txt, ic_temb, ic_img_rotary_emb)
    ...
    for block_id, block in enumerate(dit.transformer_blocks):
        ...
        if mot is not None and block_id in mot.mot_layers_mapping:
            # print(f"add mot to block {block_id}")
            c_text_hint = text_hints[mot.mot_layers_mapping[block_id]]
            c_image_hint = image_hints[mot.mot_layers_mapping[block_id]]
            text = text + c_text_hint
            image = image + c_image_hint
