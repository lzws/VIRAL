...
from .moe_lora import replace_target_modules_with_moe_lora
    ...
    def switch_pipe_to_training_mode(
        ...
        if lora_base_model is not None and not task.endswith(":data_process"):
            ...
            if lora_checkpoint is not None:
                state_dict_ = load_state_dict(lora_checkpoint)
                # state_dict = self.export_trainable_state_dict(state_dict)
                moelora_target_modules_ = moelora_target_modules.split(",")
                state_dict = {k: v for k, v in state_dict_.items() if not any(m in k for m in moelora_target_modules_)}
                state_dict = {k.replace('dit.', '').replace('.transformer', 'transformer'): v for k, v in state_dict.items() if 'text_encoder' not in k}
        ...
        #MoE Layer: transformer_blocks.0.img_mlp.net.2.weight, txt_mlp.net.2
        if moelora_base_model is not None:
            moelora_target_modules = moelora_target_modules.split(",")
            model_ = replace_target_modules_with_moe_lora(
                getattr(pipe, moelora_base_model),
                target_modules=moelora_target_modules,
                num_experts=num_experts,r=moe_lora_rank,lora_alpha=moe_lora_rank,lora_dropout=0.0,top_k=top_k,upcast_dtype=pipe.torch_dtype,moe_type=moe_type
            )

            if lora_checkpoint is not None:
                lora_moe = {k.replace('dit.', '').replace('.transformer', 'transformer'): v for k, v in state_dict_.items() if any(m in k for m in moelora_target_modules)}
                load_moe_res = model_.load_state_dict(lora_moe,strict=False)
                if len(load_moe_res[1]) > 0:
                    print(f"Warning, moe LoRA key mismatch! Unexpected keys in moe checkpoint: {load_moe_res[1]}")
                else:
                    print(f"load moe lora to train, total {len(lora_moe)} keys")
            setattr(pipe, moelora_base_model, model_)
            print(f"############# load moe lora to train ######################")
